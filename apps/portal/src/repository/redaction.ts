...
export const FIELD_RULES: Record<keyof Dispatch, FieldRule> = {
  ...
  postGraphics:   { label: 'Post graphics', disposition: 'withheld', reason: 'The rendered social graphics, one per platform — they go to the platforms with the post, not onto the archive record.' },
  ...
  adminBrief:     { label: 'Brief for the publisher', disposition: 'internal', reason: 'An admin’s instructions to a colleague. Working note, not public content.' },
  publishOverride:{ label: 'Publish override', disposition: 'internal', reason: 'Who overrode a refusal, and what they overrode. Accountability, not content.' },
  discardedReason:{ label: 'Reason discarded', disposition: 'internal', reason: 'Why a post was dropped. Internal editorial decision.' },
  revision:       { label: 'Revision count', disposition: 'internal', reason: 'How many times the post was resubmitted after being sent back. A pipeline counter, not content.' },
  screening:      { label: 'Screening record', disposition: 'withheld', reason: 'How an admin screened the raw report — counts of what was redacted and what was decided. Internal review evidence; the words removed are kept in an admin-only document, not here.' },
  request:        { label: 'Post request', disposition: 'withheld', reason: 'An admin’s brief to the publishers — goal, platforms, deadline and notes. Internal instructions about the post, not part of any public record.' },
