...
export interface HeroCarouselProps {
  ...
  className?: string
  /** Fires when any card is clicked (after focus is updated). Use to open a detail view. */
  onCardClick?: (index: number) => void
  /** Focus the carousel on mount so keyboard navigation works immediately. @default false */
  autoFocus?: boolean
}
...
export function HeroCarousel({
  ...
  return (
    ...
    >
      {/* ── Background: the focused photo, grounded in the page's own navy ── */}
      <AnimatePresence initial={false}>
        ...
        >
          ...
          />
          {/* A single, restrained navy wash — not the old full-strength
              per-station re-hue (a "color" blend at full opacity plus a
              "multiply" pass at the station accent). That made sense on
              flat generated line art, which had no real colour of its own
              to begin with, but it actively discoloured a real photograph
              — a blue-sky station photo came out the same brown/gold as
              a sepia one — and was the actual reason this hero read as a
              different visual system from the rest of the page, which is
              uniformly --ink navy underneath everything. One low-opacity
              multiply, in the page's own colour, keeps every card in that
              same family without erasing what the photo actually shows. */}
          <div
            className="absolute inset-0 opacity-40"
            style={{ backgroundColor: "var(--ink)", mixBlendMode: "multiply" }}
          />
        </motion.div>
      </AnimatePresence>
      ...
      {/* ── Headline block, sitting just above the strip's top edge ──
          `pointer-events-none`: this box runs from the very top of the
          stage (top-0) down to the strip (height: 50%) so its bottom-
          anchored text lands just above the cards — but that makes its own
          hit-test area cover the ENTIRE top-bar row underneath it too,
          since it is later in the DOM and nothing lowered its stacking.
          Real mouse clicks on Back/brand/Menu were landing on this empty
          text wrapper instead and going nowhere; a scripted `.click()` on
          the button element doesn't do hit-testing so it never showed the
          bug. Nothing in here is interactive, so disabling pointer events
          on the wrapper is free — clicks fall through to the top bar. */}
      <div
        ...
        <div className="flex w-full flex-wrap items-end gap-x-[6vw] gap-y-2">
          ...
          {active.credit ? (
            <motion.p
              ...
              // Station identity now lives here and in the position rail's
              // progress fill below, instead of re-hueing the whole photo
              // behind it — a coloured mono credit line reads as a small,
              // deliberate accent; a whole background dyed to match reads
              // as a different page.
              style={{ fontSize: label, color: accent, opacity: 0.85 }}
              initial={{ opacity: 0 }}
      ...
      >
        ...
        <div className="relative mt-2 h-px w-full bg-white/25">
          <motion.div
            ...
            style={{ width: `${100 / items.length}%`, backgroundColor: accent }}
