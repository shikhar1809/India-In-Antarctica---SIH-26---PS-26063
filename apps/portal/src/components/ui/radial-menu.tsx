/* Radial context menu — pie-slice wedges around a centre point, opened by
 * right-click.
 *
 * Adapted from the supplied component for the review annotator: the demo's
 * six-item Copy/Cut/Paste/Favorite/Pin/Delete menu is replaced by the six
 * drawing tools, and `onSelect` reports the chosen tool id rather than a
 * generic MenuItem so the caller doesn't need to know this component
 * exists to use one. The geometry, animation and Base UI wiring are
 * unchanged from the original — this is still a real @base-ui-components
 * ContextMenu underneath, not a bespoke SVG click zone, so it keeps focus
 * management and escape-to-close for free.
 *
 * The one behavioural note worth stating: Base UI's ContextMenu owns the
 * right-click. Outside this component, right-click still opens the
 * browser's normal context menu — this only intercepts it inside whatever
 * `children` wraps.
 */
...
type RadialMenuProps = {
  ...
  /** Which item, if any, should read as "currently active" — the tool in
   *  use, highlighted even before the pointer hovers a wedge. */
  activeId?: string | null;
};

type Point = { x: number; y: number };

const menuTransition: Transition = { type: 'spring', stiffness: 420, damping: 32, mass: 1 };
const wedgeTransition: Transition = { duration: 0.05, ease: 'easeOut' };
            ...
            >
              ...
              >
                <svg className="absolute inset-0 size-full" viewBox={`${-radius} ${-radius} ${radius * 2} ${radius * 2}`}>
                      ...
                      >
                        <motion.path
                          ...
                          className={cn('fill-neutral-800', {
                            'fill-neutral-700': isHover,
                            '!fill-sky-700': isActive,
                          })}
                          initial={false}
                        ...
                        <motion.path
                          ...
                          className={cn(
                            'stroke-neutral-600 stroke-1 fill-neutral-800',
                            {
                              'fill-neutral-700': isHover,
                              '!fill-sky-700': isActive,
                            },
                        ...
                        <foreignObject x={iconX - ICON_BOX / 2} y={iconY - ICON_BOX / 2} width={ICON_BOX} height={ICON_BOX}>
                          <ContextMenu.Item
                            ...
                            className={cn(
                              'size-full flex items-center justify-center rounded-full outline-none text-neutral-400',
                              { 'text-neutral-50': isHover || isActive },
                            )}
                          >
                  ...
                  <circle cx={0} cy={0} r={centerRadius} className="fill-neutral-950 stroke-1 opacity-50 stroke-neutral-600" />
                  <circle cx={0} cy={0} r={3} className="fill-none stroke-neutral-600" />
                </svg>
              </ContextMenu.Popup>
            </ContextMenu.Positioner>
          </ContextMenu.Portal>
