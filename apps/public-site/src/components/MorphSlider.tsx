...
const DEFAULT_ITEMS = [
  {
    image: 'https://images.unsplash.com/photo-1518002054494-3a6f94352e9d?w=1600&auto=format&fit=crop',
    caption: 'Maitri Research Station'
  },
  {
    image: 'https://images.unsplash.com/photo-1596720275812-4ebf8de2f31d?w=1600&auto=format&fit=crop',
    caption: 'Bharati Research Station'
  },
  {
    image: 'https://images.unsplash.com/photo-1517431265886-f682855cf649?w=1600&auto=format&fit=crop',
    caption: 'Polar Expeditions'
  }
];
...
class MorphEngine {
  ...
  boundLoop: any;
  raf: any;
  onContextLostCallback: any;

  constructor(
    container: any,
    { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap, antialias, onContextLost }: any
  ) {
  ...
  onContextLost(e: any) {
    // `preventDefault()` only stops the browser from permanently discarding
    // the context — it does nothing to actually recover rendering. Without
    // notifying the React side, a lost context (common on mobile: GPU
    // memory pressure, thermal throttling, the tab backgrounding) leaves
    // this canvas frozen on whatever frame it last drew, forever, with no
    // visible error — which is indistinguishable from "the gallery
    // animation doesn't work" on a phone that never lost the context on
    // desktop testing. The callback tells the React wrapper to tear down
    // and recreate this whole engine against a fresh context.
    e.preventDefault();
...
export default function MorphSlider({
  ...
  // Bumped when the WebGL context is lost, to force the effect below to
  // tear down and recreate the engine — see onContextLost's comment.
  const [engineGeneration, setEngineGeneration] = useState(0);
  // A phone GPU pays for this shader's chromatic-aberration/noise passes
  // at every output pixel, so a hi-DPI phone (3x devicePixelRatio is
  // common) asking for dpr-2 rendering is the single most likely cause of
  // "the animation doesn't work" actually meaning "it's too slow to look
  // like an animation" — mirrors the same isNarrow-driven dpr/antialias
  // cut the 3D gallery already makes (components/ui/3d-gallery-photography.tsx).
  ...
  useEffect(() => {
    ...
    const engine = new MorphEngine(containerRef.current, {
      ...
      onContextLost: () => setEngineGeneration(g => g + 1)
    });
  ...
  }, [items, startIndex, isNarrow, engineGeneration]);
  ...
  const hasCaptions = items.some((i: any) => i.caption);
