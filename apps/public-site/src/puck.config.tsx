...
export const config: Config<Props> = {
  components: {
        ...
        <div className="block-stats">
          <div className="stats-grid">
            {[
              ...
              <div key={i} className="stat-item flex flex-col items-center gap-4">
                <div className="stat-number">
                  <SplitFlapText 
                    words={[' ', String(s.n || '').toUpperCase()]}
                    cycleDelay={400}
                    flipDuration={0.12}
                    stagger={0.06}
                    ...
                    gap={4}
                    fontSize={48}
                    padTo={String(s.n || '').length}
                    loop={false}
                  />
                </div>
                <div className="stat-label">
                  <SplitFlapText 
                    words={[' ', String(s.l || '').toUpperCase()]}
                    cycleDelay={400}
                    flipDuration={0.08}
                    stagger={0.03}
                    charset="alpha"
    ...
    ParallaxLegacyBlock: {
      ...
      // Same fields as TextBlock (this block replaces it in the live
      // splice done in Home.tsx) â€” a scroll-linked layered reveal instead
      // of a plain paragraph, built from three real station photographs
      // rather than the demo's stock CDN images. The flag-over-Maitri shot
      // is the slowest layer on purpose: it's the one still standing once
      // the scroll settles, which is the actual "reveal" the effect is
      // building toward.
      render: ({ label, heading, body }) => (
        <ParallaxComponent
          eyebrow={label}
          title={heading}
          subtitle="43 expeditions. Two stations. One flag, flown since 1981."
          body={body}
          frontImage={{ src: '/photos/dakshin-station.jpg', alt: 'Dakshin Gangotri station', label: 'Dakshin Gangotri', info: "India's first Antarctic station, built 1983 on the Princess Astrid Coast. Decommissioned 1990." }}
          midImage={{ src: '/photos/bharati-station.jpg', alt: 'Bharati research station', label: 'Bharati Station', info: 'Built from 134 shipping containers on the Larsemann Hills, operational since 2012.' }}
          backImage={{ src: '/photos/maitri-flag.jpg', alt: 'The Indian flag carried by helicopter over Maitri station, 2005', label: 'Maitri Station', info: "India's main research base, Schirmacher Oasis, standing since 1989." }}
        />
      ),
    },
