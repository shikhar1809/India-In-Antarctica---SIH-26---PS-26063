/**
 * Seeds the Arctic reference records into the publicArchive collection.
 *
 * Same mechanism as seed-historical.mjs (read that one for how the
 * credentials work); the records themselves are in
 * apps/portal/src/repository/arcticRecords.ts, written from NCPOR's own
 * published pages and cited there.
 *
 *   node scripts/seed-arctic.mjs
 *
...
execFileSync('npx', ['esbuild', join(ROOT, 'apps', 'portal', 'src', 'repository', 'arcticRecords.ts'),
...
const { ARCTIC_RECORDS: HISTORICAL_RECORDS } = await import(pathToFileURL(bundle).href);

const token = await accessToken();

console.log(`\nSeeding ${HISTORICAL_RECORDS.length} Arctic reference records as the signed-in CLI account…\n`);
